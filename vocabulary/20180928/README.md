...
- 理論上越多的磁碟效能就等於[單一磁碟效能]×[磁碟數]，但實際上受限於匯流排I/O限制及其它因素的影響，RAID 效能會隨邊際遞減，也就是說，假設一個磁碟的效能是50MB／秒，兩個磁碟的RAID 0效能約96MB／秒，三個磁碟的RAID 0也許是130MB／秒而不是150MB／秒，所以兩個磁碟的RAID 0最能明顯感受到效能的提升
- 如果是以軟體方式來實作RAID，則磁碟的空間則不見得受限於此（例如Linux Software RAID），透過軟體實作可以經由不同的組合而善用所有的磁碟空間
